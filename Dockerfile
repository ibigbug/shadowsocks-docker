...
RUN yes | apt-get install $BUILD_DEPS --no-install-recommends

RUN mkdir -p /src/libcork && \
...
RUN yes | apt-get remove $BUILD_DEPS && apt-get clean && yes | apt-get auto-remove
